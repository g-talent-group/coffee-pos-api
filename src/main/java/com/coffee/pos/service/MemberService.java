...
public class MemberService {
    ...
    public List<MemberResponseDTO> findMemberByName(String name, boolean includeOrder) {
        return memberRepository.findByNameContainingIgnoreCase(name).stream().map(member -> MemberResponseDTO.toDTO(member, includeOrder)).toList();


    }

    public MemberResponseDTO findMemberById(Long id, boolean includeOrder) {
        Member member = memberRepository.findById(id).orElseThrow(() -> new RuntimeException("Member not found with id: " + 1d));
